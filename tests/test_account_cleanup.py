...
class AccountCleanupTests(unittest.TestCase):
    def test_deleting_account_removes_all_owned_json_files(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            history_base = data_directory / "provider_history.json"
            ...
            owned_files = (
                ...
                account_history_file(history_base, account_id),
            )
            for filename in owned_files:
                Path(filename).write_text("{}", encoding="utf-8")

            failures = remove_account_data_files(
                account_id, cache_base, favorites_base, preferences_base, history_base
            )
